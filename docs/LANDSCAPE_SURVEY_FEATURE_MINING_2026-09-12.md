...
As of 13 September 2026, current `main` includes the passive landscape tranche, authenticated supported-server hardening, bounded Parquet interchange, reviewed CAN-database interchange, evidence-bound MDF4 raw-CAN import, and signed vehicle-profile/evidence plus multi-vehicle fleet synchronization. The signed profile/fleet feature set itself entered `main` in merge commit `f414042c7bcec6c56707f74a12fca2d739359c1e`; later documentation-only commits do not change that audited feature baseline.
