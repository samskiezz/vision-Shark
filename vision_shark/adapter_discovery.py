...
import ipaddress,json,re,socket,struct,subprocess,sys
from dataclasses import asdict,dataclass,field

DOIP_PORT=13400;DOIP_VERSION=0x02

@dataclass(frozen=True)
class AdapterCandidate:
    transport:str;interface:str;confidence:float;detail:str;endpoint:str|None=None;logical_address:int|None=None;vin:str|None=None;eid:str|None=None;usable:bool=True;metadata:dict=field(default_factory=dict)
    def as_dict(self):return asdict(self)

def _linux_interfaces():
    try:rows=json.loads(subprocess.run(['ip','-json','address','show'],capture_output=True,text=True,timeout=3,check=True).stdout)
    except (OSError,subprocess.SubprocessError,json.JSONDecodeError):return []
    out=[]
    for row in rows:
        name=row.get('ifname','')
        if name=='lo' or row.get('operstate') not in ('UP','UNKNOWN'):continue
        ipv4=[{'address':i['local'],'prefixlen':int(i.get('prefixlen',24))} for i in row.get('addr_info',[]) if i.get('family')=='inet' and i.get('local')]
        out.append({'name':name,'ipv4':ipv4})
    return out

def _windows_interfaces():
    ps="Get-NetIPAddress -AddressFamily IPv4 | Where-Object {$_.IPAddress -ne '127.0.0.1' -and $_.AddressState -ne 'Duplicate'} | Select-Object InterfaceAlias,InterfaceIndex,IPAddress,PrefixLength | ConvertTo-Json -Compress"
    try:
    ...
    for row in rows:
        ...
        if addr:grouped.setdefault(name,[]).append({'address':addr,'prefixlen':int(row.get('PrefixLength') or 24)})
    return [{'name':k,'ipv4':v} for k,v in grouped.items()]

def _prefix_from_hex_netmask(value):
    try:return (int(value,16)&0xffffffff).bit_count()
...
def _broadcasts(address,prefixlen):
    try:return [str(ipaddress.ip_interface(f'{address}/{prefixlen}').network.broadcast_address),'255.255.255.255']
    except ValueError:return ['255.255.255.255']

def _parse_doip_identification(packet):
    ...
    vin=payload[:17].decode('ascii','replace').strip('\x00 ');logical=struct.unpack('!H',payload[17:19])[0]
    return {'vin':vin or None,'logical_address':logical,'eid':payload[19:25].hex(),'gid':payload[25:31].hex(),'further_action':payload[31] if len(payload)>31 else None,'protocol_version':version}

def discover_doip(timeout_s=.75):
    if timeout_s<=0 or timeout_s>10:raise ValueError('DoIP discovery timeout must be >0 and <=10 seconds')
    header=struct.pack('!BBHI',DOIP_VERSION,DOIP_VERSION^0xff,0x0001,0);found=[];seen=set();probes=[]
    for iface in network_interfaces():
        for item in iface.get('ipv4',[]):probes.append((iface['name'],item['address'],_broadcasts(item['address'],int(item.get('prefixlen',24)))))
    ...
    for iface_name,local,targets in probes:
        ...
        try:
            ...
            while True:
                ...
                seen.add(key);found.append(AdapterCandidate('doip',iface_name,.75,'ISO 13400 identification response; not vehicle identity evidence',addr[0],parsed['logical_address'],parsed['vin'],parsed['eid'],True,{'gid':parsed['gid'],'further_action':parsed['further_action'],'protocol_version':parsed['protocol_version'],'local_address':local,'transmitted':True}))
        except OSError:pass
        finally:sock.close()
    return found

def discover_j2534_registry():
    ...
    for path in roots:
        ...
        with key:
            ...
            while True:
                ...
                seen.add(marker);out.append(AdapterCandidate('j2534',str(values.get('Name') or name),.60,'Installed SAE J2534 provider',dll or None,usable=False,metadata={'vendor':values.get('Vendor'),'protocols':values.get('ProtocolsSupported'),'reason':'inventory only; passive behavior not proven'}))
    return out

def discover_adapters(include_doip=False,include_j2534=True):
    from .transports import list_can_interfaces
    out=[]
    for item in list_can_interfaces(allow_vcan=False):
        if item.get('passive_eligible') and item.get('up'):out.append(AdapterCandidate('socketcan',item['name'],1.0,'driver-reported passive CAN/CAN-FD interface',usable=True,metadata={'ctrlmode':item.get('ctrlmode',[])}).as_dict())
