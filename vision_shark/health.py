...
def build_health_report(runtime,orchestrator,audit=None,max_frame_age_s:float=2.0)->dict:
    checks=[];status=runtime.status();session=orchestrator.status()
    ...
    if source=='doip':
        ...
        checks.append(HealthCheck('simulation','ok' if status.get('connected') else 'error','software simulator connected' if status.get('connected') else 'simulator inactive'))
    else:
        checks.append(HealthCheck('vehicle_connection','degraded','no active vehicle or simulation session',required=False))
    if status.get('connected') and source in ('socketcan','simulator'):
        age=status.get('last_frame_age_s')
        fresh=age is not None and age<=max_frame_age_s
        checks.append(HealthCheck('frame_freshness','ok' if fresh else 'degraded',f'last frame age {age!r}s',required=False))
        drops=int(status.get('receive_drops') or 0);checks.append(HealthCheck('capture_integrity','ok' if drops==0 else 'degraded',f'kernel/interface receive drops: {drops}',required=False))
    ...
    return {'status':overall,'checks':rows,'vehicle_ready':session.get('state')=='ready','diagnostics_read':bool(session.get('capabilities',{}).get('diagnostics_read')),'live_actuation':False}
