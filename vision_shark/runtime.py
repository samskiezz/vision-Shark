...
class Runtime:
    ...
    def configure_decoder(self,decoder:LiveDecoder|None):
        with self._lock:
            ...
            self.decoder=decoder;self.signals.clear();self.decode_errors=0
    ...
    def _connect(self,source,kind,interface):
        ...
        with self._lock:
            self.recent.clear();self.frames_seen=0;self.decode_errors=0;self.source=source;self.source_kind=kind;self.interface=interface;self.running=True;self.last_error=None;self.last_frame_monotonic=None;self.receive_drops=int(getattr(source,'dropped',0));self._recording_drop_baseline=None;self._thread=threading.Thread(target=self._loop,name='vision-shark-rx',daemon=True);self._thread.start()
    def start_recording(self,metadata:dict|None=None)->int:
        with self._lock:
            ...
            baseline=int(self.receive_drops);meta=dict(metadata or {});meta['receive_drops_at_start']=baseline;meta['timestamp_domain']='monotonic';self.recording_id=self.storage.start_recording(self.source_kind,self.interface,meta);self._recording_drop_baseline=baseline;return self.recording_id
    def stop_recording(self):
        with self._lock:
            rid=self.recording_id;self.recording_id=None;drops=int(self.receive_drops);baseline=self._recording_drop_baseline;self._recording_drop_baseline=None
        if rid is not None and self.storage is not None:
            baseline=drops if baseline is None else int(baseline);delta=max(0,drops-baseline)
            self.storage.stop_recording(rid,{'receive_drops_at_stop':drops,'receive_drops_during_recording':delta,'capture_complete':delta==0})
        return rid
    def disconnect(self):
        self.stop_recording()
        ...
        with self._lock:self.source_kind=None;self.interface=None;self.signals.clear();self._recording_drop_baseline=None
    def _loop(self):
        while self.running:
            source=self.source
