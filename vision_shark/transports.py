...
import json,math,re,socket,struct,subprocess,sys,time
...
def list_can_interfaces(allow_vcan=False):
    ...
    for row in rows:
        ...
        ctrl=info.get('info_data',{}).get('ctrlmode',[]);flags=ctrl.get('flags',[]) if isinstance(ctrl,dict) else ctrl
        if isinstance(flags,str):flags=flags.split(',')
        flags={str(x).upper().replace('_','-') for x in (flags or [])};passive=(kind=='can' and 'LISTEN-ONLY' in flags) or (kind=='vcan' and allow_vcan)
        out.append({'name':row.get('ifname',''),'kind':kind,'up':'UP' in row.get('flags',[]),'passive_eligible':passive,'ctrlmode':sorted(flags)})
    return out

class SimulationSource:
    def __init__(self):self.step=0;self.base=time.monotonic_ns();self.dropped=0
    def open(self):return None
    def read_batch(self):
        time.sleep(.05);self.step+=1;t=self.step*.05;speed=max(0,48+18*math.sin(t/10));data=struct.pack('<HhBBBB',round(speed*100),0,self.step%16,0,0,0)
        return [Frame(ts_ns=self.base+self.step*50_000_000,bus='sim0',arbitration_id=0x601,data=data.hex())]
    def read(self,timeout=.25):return self.read_batch()[0]
    def close(self):return None
SimulatorSource=SimulationSource

class SocketCanSource:
    def __init__(self,interface,allow_vcan=False):
        self.interface=interface;self.details=interface_details(interface);self.assurance=require_passive(self.details,allow_vcan);self.dropped=0;self.sock=None
        self._open()
    def _open(self):
        self.sock=socket.socket(socket.AF_CAN,socket.SOCK_RAW,socket.CAN_RAW);self.sock.setsockopt(SOL_CAN_RAW,CAN_RAW_FD_FRAMES,1);self.sock.settimeout(.15);self.sock.bind((self.interface,))
    def open(self):return None
    def read_batch(self):
        raw=self.sock.recv(72);can_id,length,flags,_,_=struct.unpack('=IBBBB',raw[:8]);fd=len(raw)==72;rtr=bool(can_id&CAN_RTR_FLAG)
        return [Frame(ts_ns=time.monotonic_ns(),bus=self.interface,arbitration_id=can_id&0x1fffffff,data='' if rtr else raw[8:8+length].hex(),can_fd=fd,extended=bool(can_id&CAN_EFF_FLAG),rtr=rtr,error=bool(can_id&CAN_ERR_FLAG),brs=fd and bool(flags&1),esi=fd and bool(flags&2))]
